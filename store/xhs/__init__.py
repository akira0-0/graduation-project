...
def get_video_url_arr(note_item: Dict) -> List:
    ...
    videoArr = []
    try:
        consumer_info = video_info.get('consumer', {})
        originVideoKey = consumer_info.get('origin_video_key', '')
        if not originVideoKey:
            originVideoKey = consumer_info.get('originVideoKey', '')
        
        # Fallback with watermark
        if not originVideoKey:
            media_info = video_info.get('media', {})
            stream_info = media_info.get('stream', {})
            videos = stream_info.get('h264', [])
        ...
        utils.logger.warning(f"[get_video_url_arr] Failed to extract video URL: {e}")
        return []

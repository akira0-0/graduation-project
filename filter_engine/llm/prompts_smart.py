...
- **select（筛选保留）**：命中即保留，**满足其中任意一个关键词即可通过**。用于保留想要的内容，如特定主题、质量标准
  * 示例：select 规则含 ["黄金", "金价", "走势"]，内容只需包含 "黄金" 即可通过
  * 关键词越多，覆盖面越广，通过率越高

## ⭐ 强制要求：主题锚定规则（每次必须执行）
**无论规则库是否已有相关规则，你都必须从用户 query 中提取 1 个最核心的实体关键词（地名/品牌/核心概念），
生成一条 purpose=select 的"主题锚定规则"，放入 gap_rules。**
- 这条规则确保只有与主题直接相关的内容才能通过 Layer-2
- 示例："丽江旅游攻略" → 提取 ["丽江"]；"特斯拉Model3购车体验" → 提取 ["特斯拉", "Model3", "model 3"]
- 关键词要涵盖常见变体（繁体、缩写、英文），但不要太宽泛
- 这条规则必须是 gap_rules 数组的第一条

## 规则格式
- type: keyword（关键词列表匹配，OR 逻辑）| regex（正则表达式匹配）| pattern（组合逻辑）
...
### Step 1：意图识别 + 主题锚定（必须完成）
- **1a 实体提取（必须）**：从用户 query 中提取 1 个最核心的实体关键词（地名/品牌/核心概念及其同义/变体词），
  立即在 thought_trace.step_1_extraction 中记录，并生成一条 purpose=select 的"主题锚定规则"放入 gap_rules 首位。
  * "丽江旅游攻略" → 主题锚定词: ["丽江", "古城"] → gap_rules 第1条: select rule
  * "特斯拉购车体验" → 主题锚定词: ["特斯拉", "Tesla"] → gap_rules 第1条: select rule
  * "黄金投资策略" → 主题锚定词: ["黄金", "金价", "gold"] → gap_rules 第1条: select rule
- **1b 约束提取**：提取所有其他过滤/保留约束，标注每条约束是 filter 还是 select
...
{{
  ...
  "gap_rules": [
    {{
      "name": "社交-质量-高质量评论词",
      "type": "keyword",
      "content": ["写得很好", "非常有用", "干货满满", "亲测有效", "感谢分享", "收藏了", "学到了", "受益匪浅", "值得推荐", "赞"],
      "category": "other",
      "purpose": "select",
...
**示例说明**：
- ✅ 好的规则：包含 **8-15 个关键词**，覆盖面广，宽松筛选
- ❌ 不好的规则：只有 2-3 个关键词，过于严格，容易遗漏
```
"""
